		...
		if (zaxxon_vid_type == CONGO_VID)
		{
			...
			for (i=0x1e ; i>=0; i--)
			{
				...
				if (spriteram.read(offs+2)!= 0xff)
				{
					...
					sx = ((spriteram.read(offs+2+3)+ 16) & 0xff) - 31;
					sy = 255 - spriteram.read(offs+2)- 15;
					flipx = spriteram.read(offs+2+2)& 0x80;
					flipy = spriteram.read(offs+2+1)& 0x80;
		...
		else if (zaxxon_vid_type == FUTSPY_VID)
		{
			for (offs = spriteram_size - 4;offs >= 0;offs -= 4)
			{
				if (spriteram.read(offs)!= 0xff)
				{
					...
					flipx = spriteram.read(offs+1)& 0x80;
